...
# Section Visibility Control functions
# Function that returns updates for the visibility of sections.
def switch_section(selected):
    return (
        gr.update(visible=(selected == "Help")),
        gr.update(visible=(selected == "Model and Parameter Selection")),
        gr.update(visible=(selected == "Main Workflow")),
        gr.update(visible=(selected == "Named Entity Recognition")),
        gr.update(visible=(selected == "Query Text")),
        gr.update(visible=(selected == "Run Aggregation"))
    )

# Helper callbacks for each button.
def on_help():
    return switch_section("Help")

def on_model_select():
    return switch_section("Model and Parameter Selection")

def on_main():
    return switch_section("Main Workflow")

def on_ner():
    return switch_section("Named Entity Recognition")

def on_query():
    return switch_section("Query Text")

def on_aggregate():
    return switch_section("Run Aggregation")

    
# Gradio Interface
# check theming guide: https://www.gradio.app/guides/theming-guide
with gr.Blocks(title="Thematic Analyzer", theme=gr.themes.Dark()) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=180):
            ...
            model_select_btn = gr.Button("🤖 Model and Parameter Selection")
            main_btn = gr.Button("📝 Main Workflow")
            ner_btn = gr.Button("📌 Named Entity Recognition")
            query_btn = gr.Button("💬 Query Your Text")
            aggregate_btn = gr.Button("🗃️ Aggregate Runs")
    
        # Main content column
        with gr.Column(scale=4):
            #### Help Section ####
            help_section = gr.Column(visible=True)
            with help_section:
                gr.Markdown("# 👋 Welcome!")
                gr.Markdown("This is an app to help with finding and analyzing themes in texts and transcripts.")
                gr.Markdown('''The basic workflow of the app is pretty simple:
                
                1. 🤖 set model and chunking parameters if desired
                
                2. 📁 load a document
                
                3. 🏷️ use an LLM to code the document
                ...
                query the text, and, if you have coded, clustered,
                or summarized multiple times, aggregate and analyze the results.''')

            #### Model and Parameter Selection ####
            model_param_selection = gr.Column(visible=False)
            with model_param_selection:
                gr.Markdown("### 🤖 Model and Text Chunking Parameters")
            ...
            main_workflow = gr.Column(visible=False)
            with main_workflow:
                    ...
                    with gr.Tab("📝 Summarize Themes"):
                        ...
                        with gr.Accordion("More Visualizations", open=False):                                
                            with gr.Tab("📊 Sankey Plot"):
                                ...
                                sankey_button = gr.Button("📊 Visualize Sankey")
                            ...
                            with gr.Tab("☁️ Word Cloud"): 
                                ...
                                make_cloud_button = gr.Button("☁️ Make Word Cloud")
            ...
            ner_section = gr.Column(visible=False)
            with ner_section:
            ...
            query_section = gr.Column(visible=False)
            with query_section:
                gr.Markdown("### 💬 Query Your Text")
                with gr.Row():
                ...
                chat_button.click(
                    fn=ta.open_chat,
                    inputs=[],
                    outputs=[chatbot_box, user_msg, send_btn])
            ...
            #### Run Aggregation ####
            aggregate_section = gr.Column(visible=False)
            with aggregate_section:
                gr.Markdown("### 🗃️ Aggregate Runs")
                ...
                with gr.Tabs():
                    with gr.Tab("Aggregation Management"):
                    ...
                    with gr.Tab("Aggregation Analysis"):
                        with gr.Tabs():
    ...
    # --- Core Button Logic ---
    # Set up button callbacks to switch sections:
    help_btn.click(fn=on_help, inputs=[], outputs=[help_section, model_param_selection, main_workflow, ner_section, query_section, aggregate_section])
    model_select_btn.click(fn=on_model_select, inputs=[], outputs=[help_section, model_param_selection, main_workflow, ner_section, query_section, aggregate_section])
    main_btn.click(fn=on_main, inputs=[], outputs=[help_section, model_param_selection, main_workflow, ner_section, query_section, aggregate_section])
    ner_btn.click(fn=on_ner, inputs=[], outputs=[help_section, model_param_selection, main_workflow, ner_section, query_section, aggregate_section])
    query_btn.click(fn=on_query, inputs=[], outputs=[help_section, model_param_selection, main_workflow, ner_section, query_section, aggregate_section])
    aggregate_btn.click(fn=on_aggregate, inputs=[], outputs=[help_section, model_param_selection, main_workflow, ner_section, query_section, aggregate_section])
